...
class TokenAnalyzer(object):
    ...
    def process_data(self):
        ...
        self.contribution_data = self.contribution_data.merge(self.repo_dictionary_data[['repo_full_name','symbol']], how='left')
    ...
    def filter_token_contribution(self, 
        ...
        if symbols <> None:
            self.contribution_data_filter = self.contribution_data_filter[self.contribution_data_filter.symbol.isin(symbols)]
        else:
            pass
        if developers <> None:
            self.contribution_data_filter = self.contribution_data_filter[self.contribution_data_filter.developer.isin(developers)]
        else:
            pass

        if repo_full_names <> None:
            self.contribution_data_filter = self.contribution_data_filter[self.contribution_data_filter.repo_full_name.isin(repo_full_names)]
        else:
            pass
    ...
    def get_commit_leaderboard(self, dimension=['symbol'], limit=20, *args,**kwargs):
        ...
        self.commit_leaderboard = self.contribution_data_filter.groupby(dimension).commits.sum().sort_values(ascending=False)[0:limit]
...
data_contributions_entry = data_contributions_entry[~data_contributions_entry.repo_full_name.isin(['input-output-hk/nixpkgs'])]
...
# generate charts
token_class.get_commit_leaderboard(dimension=['developer'], symbols=['ADA'], limit=20, time_range=90).sort_values().plot(kind='barh')


# In[23]:


token_class.plot_commit_leaderboard(dimension=['symbol'], limit=20).sort_values().plot(kind='barh', title='last_week_top_committed_coin')
...
fig = plt.figure()
token_class.plot_commit_leaderboard(dimension=['developer'], limit=20).sort_values().plot(kind='barh', title='last_week_top_committed_developer')
...
token_contribution_2016 = token_class.filter_token_contribution(end_time=datetime.date(2016,12,31), time_range=365)
token_matrix_2016 = token_contribution_2016.pivot_table(index='symbol', columns='developer', values='commits', aggfunc=np.sum).fillna(0)


# In[10]:


token_contribution_2017 = token_class.filter_token_contribution(end_time=datetime.date(2017,12,31), time_range=365)
token_matrix_2017 = token_contribution_2017.pivot_table(index='symbol', columns='developer', values='commits', aggfunc=np.sum).fillna(0)


# In[11]:


token_influence_2016 = token_matrix_2016.dot(token_matrix_2016.T)
token_influence_2017 = token_matrix_2017.dot(token_matrix_2017.T)
...
token_influence_2017.sum().sort_values(ascending=False)


# In[15]:


token_contribution_2017.groupby('symbol').commits.sum().sort_values(ascending=False)


# In[198]:


token_influence = token_matrix.dot(token_matrix.T)


# In[199]:


token_influence.sum().sort_values(ascending=False)


# In[200]:


token_influence.loc['ETH'].sort_values(ascending=False)
