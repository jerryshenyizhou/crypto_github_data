...
token_list_1 = pd.DataFrame.from_dict(token_list, orient='index').reset_index().rename(columns={'index':'symbol',0:'github_org'})
...
for symbol in data_top_100_token_list.symbol.unique():
    for github_org in list(token_org_df[token_org_df.index==symbol].github_org):
        try:
            data_repo = getJSON("https://api.github.com/users/"+str(github_org)+"/repos?sort=updated&direction=desc&per_page=100")
...
for repo_name in token_repo_df_select.full_name.unique():
    try:
        data_repo_contributors = json_normalize(getJSON("https://api.github.com/repos/"+repo_name+"/stats/contributors?sort=total&direction=desc&per_page=100"))
        data_repo_contributors['repo_full_name'] = repo_name
        data_repo_contributors = data_repo_contributors.dropna(subset=['author.login']).set_index(['repo_full_name','author.login'])[['weeks']]
        ...
        minutes_passed = (datetime.datetime.today() - start_time).seconds/60
        print 'repo '+repo_name+' flattern completed! used '+str(minutes_passed)+' minutes! '+'memory used '+str(memory)+'MB'
        del data_repo_contributors

    except:
        print 'repo '+repo_name+' flattern failed! used '+str(minutes_passed)+' minutes! '+'memory used '+str(memory)+'MB'
        pass
...
data_contributions_entry = data_contributions_entry.rename(columns={'w':'week','c':'commits','a':'additions','d':'deletions','author.login':'login'})

data_contributions_merge = data_contributions_entry.merge(token_repo_df[['full_name','symbol']].rename(columns={'full_name':'repo_full_name'}), on='repo_full_name')
